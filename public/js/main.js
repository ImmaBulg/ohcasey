...
$(document).ready(function () {
    ...
    $('.collapseBtn').on('click', '.fa-plus', function() {
        console.log('test');
        $(this).removeClass('fa-plus').addClass('fa-minus');
    });

    $('.collapseBtn').on('click', '.fa-minus', function() {
        /*console.log('test');*/
        $(this).removeClass('fa-minus').addClass('fa-plus');
    });

    $('.js-select-item-device + .select2 .select2-selection__rendered').attr('title', ($('.select2-selection__rendered').attr('title') + " выберите вашу модель телефона в списке для подбора Вашего чехла"));
    $('.js-select-item-device').on('change', function() {
        $('.select2-selection__rendered').attr('title', ($('.select2-selection__rendered').attr('title') + " выберите вашу модель телефона в списке для подбора Вашего чехла"));
    });
    /*js-select-item-case*/
    console.log($('.js-select-item-case').next());
    $('.js-select-item-case ~ .select2 .select2-selection__rendered').attr('title', ($('.select2-selection__rendered').attr('title') + " выберите свой чехол в списке для подбора Вашего чехла"));
    $('.js-select-item-case').on('change', function() {
        $('.select2-selection__rendered').attr('title', ($('.select2-selection__rendered').attr('title') + " выберите свой чехол в списке для подбора Вашего чехла"));
    });
...
$(function () {
    ...
    $('.js-burger').on('click', function (e) {
        ...
        $('.js-nav-drop').slideUp('fast');
        $('.js-nav-link').removeClass('is-opened');
        $('.header').toggleClass('header-close-btn');
        e.preventDefault();
    });
    ...
    if ($('.js-select-item').length) {
        ...
        $('.js-select-item').on('change', function (evt) {
            if (typeof(glitter_page) !== "undefined") {
                if (window.location.pathname.split('/').indexOf('product') != -1) {
                    if (window.location.search !== '') {
                        console.log(devices[$('.js-select-item').val()]);
                        console.log($('.js-select-item').val());
                        window.location.search = '?sort=&device=' + devices[$('.js-select-item').val()];
                    }
                }
            }
...
Mega.topProducts = {
	...
	init: function (){
		this.$catalog = $('.async-catalog');
		if(!this.$catalog.length) return null;
		
		this.itemsCount = this.$catalog.find('.catalog__item').length;
		this.__getBy = this.$catalog.data('async-by') ? ~~this.$catalog.data('async-by') : 12;
	...
	/**
	* Обработчик на скролл, который будет подтягивать товары
	*/
	handleWindowScroll: function (){
		var self = Mega.topProducts,
			$window = $(window);
		if($window.height() + $window.scrollTop() > self.$catalog.offset().top + self.$catalog.outerHeight()){
			self.__getProducts();
		}
	},
	...
	__getProducts(){
		var self = this;
		if(self.isLoading || self.itemsEnd) return false;
		
		self.isLoading = true;
		$.ajax({
			url: '/index-top/' + self.itemsCount,
			type: 'get',
			data: {
				'type': 'index',
				'limit': self.__getBy
			...
			success: function (products){
				...
				self.appendProducts(data.result);
				self.itemsCount += data.itemsCount;				
				self.isLoading = false;
				if(data.itemsCount < 12){
					this.itemsEnd = true;
					$(window).off('scroll', this.handleWindowScroll); //Снимаем обработчик на скролл окна, если элемнтов больше нет
				}
			}
		});
	},
